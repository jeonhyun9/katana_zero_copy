...
void enemyClass::render()
{
	for (int i = 0; i < vEnemy.size(); i++)
	{
		...
		{
			IMAGEMANAGER->findImage("enemy_follow")->render(getMemDC(), vEnemy[i].centerX - 4, vEnemy[i].rc.top - 40);
		}
		//총알렌더
		for (int j = 0; j < BULLETSIZE; j++)
		{
		...
		case E_WALK:
			...
			case E_POMP:
				...
				break;
			case E_COP:
				vEnemy[i].walkImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 3, vEnemy[i].frameIdx, vEnemy[i].frameY);
				break;
			case E_GRUNT:
				vEnemy[i].walkImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 10, vEnemy[i].frameIdx, vEnemy[i].frameY);
				break;
			}
			break;
		case E_RUN:
