		...
		for (int i = 0; i < vItem.size(); i++)
		{
			if (vItem[i].isFirstPick == false)
			{
				...
				if (vItem[i].arrow.isAvailable)
				{
					...
					{
					case I_BOTTLE:
						vItem[i].arrow.img->frameRender(getMemDC(), vItem[i].rc.left - 1, vItem[i].rc.top - 30, frameIdx, 0);
						break;
					case I_BUST:
						vItem[i].arrow.img->frameRender(getMemDC(), vItem[i].rc.left + 9, vItem[i].rc.top - 30, frameIdx, 0);
						break;
					case I_KNIFE:
						vItem[i].arrow.img->frameRender(getMemDC(), vItem[i].rc.left + 10, vItem[i].rc.top - 30, frameIdx, 0);
						break;
					}
				}
			}
