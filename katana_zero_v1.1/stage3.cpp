...
HRESULT stage3::init()
{
	...
	enemyPt->generateEnemy(1417, 1172, E_COP, E_IDLE, false); 
	objectPt->generateObject(1552, 1088, 1200, O_DOOR);
	enemyPt->generateEnemy(1600, 1172, E_BOLD, E_WALK, true);
	objectPt->generateObject(1752, 1162, 1200, O_DRUM);
	enemyPt->generateEnemy(1700, 1172, E_COP, E_WALK, false);
	objectPt->generateObject(1762, 1162, 1200, O_DRUM);
	objectPt->generateObject(1852, 1162, 1200, O_DRUM);
	enemyPt->generateEnemy(1900, 1172, E_GRUNT, E_WALK, true);
	...
	enemyPt->generateEnemy(2100, 1172, E_POMP, E_IDLE, false);
	...
	enemyPt->generateEnemy(2750, 1172, E_BOLD, E_IDLE, true); 
...
void stage3::update()
{
	...
	for (int i = 0; i < enemyPt->getVectorEnemy().size(); i++)
	{
		...
		for (int j = 0; j < objectPt->getVectorObject().size(); j++)
		{
			RECT tempRECT88;
			RECT tempRECT87;
			if (IntersectRect(&tempRECT88, &enemyPt->getEnemyRect(i), &objectPt->getObjectRect(j)) && objectPt->getObject(j).isBreak == true && playerPt->getPlayer().state == P_DOORBREAK && objectPt->getObject(j).category == O_DOOR)
			{
