		...
		if (vEnemy[i].x + 200 - DATA->getBgCol().x >= 0 && vEnemy[i].x - 200 - DATA->getBgCol().x <= WINSIZEX)
		{
			...
			case E_IDLE:
				...
				case E_COP:
					...
					break;
				case E_GRUNT:
					vEnemy[i].idleImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 1, vEnemy[i].frameIdx, vEnemy[i].frameY);
					break;
				case E_TURRET:
...
void enemyClass::enemyDoorDead(int i)
{
	...
	if (vEnemy[i].isDoorDead && vEnemy[i].isDoorDeadMove)
	{
