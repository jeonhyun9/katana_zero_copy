...
{
	RECT tempRC;
	if (IntersectRect(&tempRC, &playerPt->getPlayerRect(), &bossPt->getLazerRect()) && bossPt->getBoss().lazerHitBox==true && DATA->getDead() == false && playerPt->getPlayer().state != P_ROLL && bossPt->getBoss().state != B_HURTRECOVER && bossPt->getBoss().state != B_OUT)
	{
		playerPt->setHurt(true);
