...
void bossClass::update()
{
	...
	if (DATA->getStop() == false)
	{
		...
		{
			float tempSpeed;
			boss.isRight ? tempSpeed = 0.3f : tempSpeed = -0.3f;
			boss.x += tempSpeed;
		}
		...
		{
			boss.findB = (boss.y + boss.height / 2) + 20;
		}
		else
		{
		...
		this->bossAnimation();
		if (boss.state == B_IDLE || boss.state == B_AIM || boss.state == B_LAND || boss.state == B_DIEGROUND || boss.state == B_DEAD)
		{
			this->bossOnFloor();
		}
...
void bossClass::render()
{
		...
		for (int i = 0; i < vLazer.size(); i++)
		{
			if (vLazer[i].isGround && vLazer[i].isRotate == true)
			{
				if (boss.isRight)
				{
					...
					{
						//vLazer[i].img->rotateFrameRender(getMemDC(), vLazer[i].x - DATA->getBgCol().x - 40, vLazer[i].y - DATA->getBgCol().y, 0, 4, vLazer[i].angle);
					}
					if (boss.frameIdx == 23)
					{
						vLazer.erase(vLazer.begin() + i);
				...
				{ 
					...
					{
						//vLazer[i].img->rotateFrameRender(getMemDC(), vLazer[i].x - DATA->getBgCol().x - 30, vLazer[i].y - DATA->getBgCol().y, 0, 4, vLazer[i].angle);
					}
					if (boss.frameIdx == 2)
					{
						vLazer.erase(vLazer.begin() + i);
	...
	if (boss.isGenrateCoolTime == false)
	{
		...
		case B_SHOOT:
			if (boss.isRight)
			{
				boss.shootImg->frameRender(getMemDC(), boss.rc.left - 22, boss.rc.top - 3, boss.frameIdx, boss.frameY);
			}
			else
			{
				boss.shootImg->frameRender(getMemDC(), boss.rc.left - 70, boss.rc.top - 3, boss.frameIdx, boss.frameY);
			}
			break;
		case B_IN:
		...
		case B_OUT:
			...
			break;
		case B_DIEGROUND:
			boss.dieGroundImg->frameRender(getMemDC(), boss.rc.left - 14, boss.rc.top + 3, boss.frameIdx, boss.frameY);
			break;
		case B_HEADGROUND:
			break;
		case B_HEADFLY:
		...
		case B_DIEFLY:
			...
			break;
		case B_NOHEAD:
			boss.noHeadImg->frameRender(getMemDC(), boss.rc.left - 14, boss.rc.top + 46, boss.frameIdx, boss.frameY);
			break;
		case B_DEAD:
...
void bossClass::setBossHit(bool _set)
{
	...
	else
	{
		...
		{
			boss.isCrashedBottom ? setBossState(B_DIEGROUND) : setBossState(B_DIEFLY);
			boss.currentPattern++;
		}
	}
}
...
void bossClass::animationSetFrame(int _frameTum, int _End)
{
	...
	{
		boss.frameY = 0;
		if (boss.state == B_HURTRECOVER && boss.isCrashedBottom == false && DATA->getCurrentClear()== STAGE4_CLEARED)
		{
			boss.frameIdx = 1;
			boss.isRight ? boss.moveAngle = PI - 1 : boss.moveAngle = 1;
			if (SLOW_inBoss == false)boss.gravity += 0.4f;
		...
		else if (boss.state == B_HURTRECOVER && boss.isCrashedBottom == true && boss.frameIdx > 12 && DATA->getCurrentClear() == STAGE4_CLEARED)
		{
			boss.gravity = 0;
			boss.stateCnt = 0;
			setBossState(B_OUT);
	...
	else
	{
		...
		else if (boss.state == B_HURTRECOVER && boss.isCrashedBottom == true && boss.frameIdx < 2 && DATA->getCurrentClear() == STAGE4_CLEARED)
		{
			boss.gravity = 0;
			boss.stateCnt = 0;
			setBossState(B_OUT);
...
void bossClass::randomPatten(int _bossX)
{
	...
	case 1120:
		...
		else setBossState(B_DRAWGUN);
		break;
	case 970:
		if (_num2 == 1)
		{
			setBossState(B_TELEPORT);
		}
		else
		{
			setBossState(B_PREJUMP);
		}
		break;
	}
