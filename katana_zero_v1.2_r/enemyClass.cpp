...
HRESULT enemyClass::init()
{
	...
	go.img = IMAGEMANAGER->findImage("hud_go_arrow");
	go.x = 1150;
	go.y = 80;
	go.isMove = true;
	go.cnt = 0;
...
void enemyClass::update()
{
	...
	else if (vEnemy.empty()==false)
	{
		...
		{
			//파인드 렉트 크기
			if (vEnemy[i].state == E_HURT && isAllDead == false)deadCnt++;
			//건스파크 좌표 수정
			if (vEnemy[i].isRight)
			{
				if ((int)vEnemy[i].distanceToPlayer_Y < -20 && (int)vEnemy[i].distanceToPlayer_Y > -50)
				{
					vEnemy[i].gunsparkX = vEnemy[i].rc.left + 95;
				...
				{
					vEnemy[i].gunsparkX = vEnemy[i].rc.left + 85;
					vEnemy[i].gunsparkY = vEnemy[i].rc.top - 10;
				}
				else
				{
			...
			else
			{
				if ((int)vEnemy[i].distanceToPlayer_Y < -20 && (int)vEnemy[i].distanceToPlayer_Y > -50)
				{
					vEnemy[i].gunsparkX = vEnemy[i].rc.left - 50;
					vEnemy[i].gunsparkY = vEnemy[i].rc.top + 10;
				}
				else if ((int)vEnemy[i].distanceToPlayer_Y < -50 && (int)vEnemy[i].distanceToPlayer_Y > -100)
				{
				...
				{
					vEnemy[i].gunsparkX = vEnemy[i].rc.left - 40;
					vEnemy[i].gunsparkY = vEnemy[i].rc.top - 10;
				}
			...
			//{
			//	vEnemy[i].gravity += 0.4f;
			//	if (vEnemy[i].isCrashedRight == false)vEnemy[i].x += cosf(vEnemy[i].deadAngle) * 10.0f;
			//	vEnemy[i].y += -sinf(vEnemy[i].deadAngle) * 10.0f + vEnemy[i].gravity;
			//}
		
			//픽셀충돌 탐색점
			vEnemy[i].findL = (vEnemy[i].x - vEnemy[i].width / 2) - 20;
			vEnemy[i].findR = (vEnemy[i].x + vEnemy[i].width / 2) + 20;
			...
			bloodAnimation(i);
			enemyAnimation(i);
			if (vEnemy[i].isDoorDead && vEnemy[i].realDead == false && vEnemy[i].isDoorDead)
			{
				enemyDoorDead(i);
			}
		}
	...
	if (deadCnt == vEnemy.size())
	{
		if (isAllDead == false && DATA->getCurrentClear()!=STAGE3_CLEARED && DATA->getCurrentClear()!=STAGE4_CLEARED && DATA->getCurrentClear()!=STAGE5_RESET)SOUNDMANAGER->play("go");
		isAllDead = true;
	}
	else
	{
...
void enemyClass::render()
{
	for (int i = 0; i < vEnemy.size(); i++)
	{
		...
		if (vEnemy[i].founded && vEnemy[i].state != E_HURT && DATA->getDead()==false)
		{
			IMAGEMANAGER->findImage("enemy_follow")->render(getMemDC(), vEnemy[i].centerX - 8, vEnemy[i].rc.top - 45);
		}
		//총알렌더
		for (int j = 0; j < BULLETSIZE; j++)
		{
		...
		if (vEnemy[i].x + 200 - DATA->getBgCol().x >= 0 && vEnemy[i].x - 200 - DATA->getBgCol().x <= WINSIZEX)
		{
			...
			case E_IDLE:
				...
				case E_POMP:
					...
					break;
				case E_COP:
					vEnemy[i].idleImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 3, vEnemy[i].frameIdx, vEnemy[i].frameY);
					break;
				case E_GRUNT:
					vEnemy[i].idleImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 4, vEnemy[i].frameIdx, vEnemy[i].frameY);
					break;
				case E_TURRET:
			...
			case E_WALK:
				...
				{
				case E_BOLD:
					vEnemy[i].walkImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 5, vEnemy[i].frameIdx, vEnemy[i].frameY);
					break;
				case E_POMP:
					vEnemy[i].walkImg->frameRender(getMemDC(), vEnemy[i].rc.left - 10, vEnemy[i].rc.top - 16, vEnemy[i].frameIdx, vEnemy[i].frameY);
					break;
				case E_COP:
					vEnemy[i].walkImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 8, vEnemy[i].frameIdx, vEnemy[i].frameY);
					break;
				case E_GRUNT:
					vEnemy[i].walkImg->frameRender(getMemDC(), vEnemy[i].rc.left - 20, vEnemy[i].rc.top - 13, vEnemy[i].frameIdx, vEnemy[i].frameY);
					break;
				}
				break;
			case E_RUN:
			...
			case E_HURT:
				...
				case E_TURRET:
					...
					{
						if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 0);
					}
					else
					{
						if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 1);
					}
					break;
				case E_BOLD:
					//플레이어의 방향에 따라 피 이미지 렌더
					if (vEnemy[i].isExplosionDead == false)
					{
						if (vEnemy[i].isPlayerRight)
						{
					...
					if (vEnemy[i].isExplosionDead == false)
					{
						...
						if (vEnemy[i].isPlayerRight)
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left + 32, vEnemy[i].rc.top - 48, vEnemy[i].frameIdxBlood, 0);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 0);
						}
						else
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left - 50, vEnemy[i].rc.top - 48, vEnemy[i].frameIdxBlood, 1);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 1);
						}
					}
					//피격 이미지 렌더
				...
				case E_GRUNT:
					...
					if (vEnemy[i].isRight)
					{
						vEnemy[i].hurtImg->frameRender(getMemDC(), vEnemy[i].rc.left - 10, vEnemy[i].rc.top - 10, vEnemy[i].frameIdx, 0);
					}
					else
					{
					...
					if (vEnemy[i].isExplosionDead == false)
					{
						...
						if (vEnemy[i].isPlayerRight)
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left + 44, vEnemy[i].rc.top - 56, vEnemy[i].frameIdxBlood, 0);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->alphaFrameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 0, 200);
						}
						else
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left - 50, vEnemy[i].rc.top - 56, vEnemy[i].frameIdxBlood, 1);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->alphaFrameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 1, 200);
						}
					}
				...
				case E_COP:
					...
					if (vEnemy[i].isRight)
					{
						vEnemy[i].hurtImg->frameRender(getMemDC(), vEnemy[i].rc.left + 5, vEnemy[i].rc.top, vEnemy[i].frameIdx, 0);
					}
					else
					{
						vEnemy[i].hurtImg->frameRender(getMemDC(), vEnemy[i].rc.left - 5, vEnemy[i].rc.top, vEnemy[i].frameIdx, 1);
					}
					//플레이어의 방향에 따라 이펙트 이미지 렌더
					if (vEnemy[i].isExplosionDead == false)
					{
						if (vEnemy[i].isPlayerRight)
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left + 50, vEnemy[i].rc.top - 50, vEnemy[i].frameIdxBlood, 0);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 0);
						}
						else
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left - 50, vEnemy[i].rc.top - 50, vEnemy[i].frameIdxBlood, 1);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 1);
						}
					}
					//피격 이미지 렌더
				...
				case E_POMP:
					...
					{
						vEnemy[i].hurtImg->frameRender(getMemDC(), vEnemy[i].rc.left - 30, vEnemy[i].rc.top - 12, vEnemy[i].frameIdx, 1);
					}
					//플레이어의 방향에 따라 이펙트 이미지 렌더
					if (vEnemy[i].isExplosionDead == false)
					{
						if (vEnemy[i].isPlayerRight)
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left + 30, vEnemy[i].rc.top - 56, vEnemy[i].frameIdxBlood, 0);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 0);
						}
						else
						{
							vEnemy[i].bloodImg->frameRender(getMemDC(), vEnemy[i].rc.left - 52, vEnemy[i].rc.top - 56, vEnemy[i].frameIdxBlood, 1);
							if (vEnemy[i].isReflectDead == false && vEnemy[i].isDoorDead == false)vEnemy[i].slashHitImg->frameRender(getMemDC(), vEnemy[i].rc.left - 27, vEnemy[i].rc.top - 26, vEnemy[i].frameIdxHitSlash, 1);
						}
					}
					//피격 이미지 렌더
			...
			case E_AIM:
				...
				case E_BOLD:
					...
					{
						vEnemy[i].leftarm_R->rotateRender(getMemDC(), vEnemy[i].rc.left + 35, vEnemy[i].rc.top + 23, vEnemy[i].angleToPlayer - PI);
						vEnemy[i].aimImg_R->render(getMemDC(), vEnemy[i].rc.left, vEnemy[i].rc.top);
						vEnemy[i].gunImg_R->rotateRender(getMemDC(), vEnemy[i].rc.right + 5, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer - PI);
						vEnemy[i].rightarm_R->rotateRender(getMemDC(), vEnemy[i].rc.left + 20, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer - PI);
					...
					{
						vEnemy[i].leftarm_L->rotateRender(getMemDC(), vEnemy[i].rc.left + 10, vEnemy[i].rc.top + 23, vEnemy[i].angleToPlayer);
						vEnemy[i].aimImg_L->render(getMemDC(), vEnemy[i].rc.left, vEnemy[i].rc.top);
						vEnemy[i].gunImg_L->rotateRender(getMemDC(), vEnemy[i].rc.left + 10, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer);
						vEnemy[i].rightarm_L->rotateRender(getMemDC(), vEnemy[i].rc.left + 25, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer);
				...
				case E_COP:
					...
					{
						vEnemy[i].leftarm_R->rotateRender(getMemDC(), vEnemy[i].rc.left + 35, vEnemy[i].rc.top + 23, vEnemy[i].angleToPlayer - PI);
						vEnemy[i].aimImg_R->render(getMemDC(), vEnemy[i].rc.left, vEnemy[i].rc.top - 3);
						vEnemy[i].gunImg_R->rotateRender(getMemDC(), vEnemy[i].rc.right + 5, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer - PI);
						vEnemy[i].rightarm_R->rotateRender(getMemDC(), vEnemy[i].rc.left + 20, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer - PI);
						if (vEnemy[i].isFire)vEnemy[i].gunsparkImg->rotateFrameRender(getMemDC(), vEnemy[i].gunsparkX, vEnemy[i].gunsparkY, vEnemy[i].frameIdx, vEnemy[i].frameY, vEnemy[i].angleToPlayer - PI);
					}
					...
					{
						vEnemy[i].leftarm_L->rotateRender(getMemDC(), vEnemy[i].rc.left + 10, vEnemy[i].rc.top + 23, vEnemy[i].angleToPlayer);
						vEnemy[i].aimImg_L->render(getMemDC(), vEnemy[i].rc.left, vEnemy[i].rc.top - 3);
						vEnemy[i].gunImg_L->rotateRender(getMemDC(), vEnemy[i].rc.left + 10, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer);
						vEnemy[i].rightarm_L->rotateRender(getMemDC(), vEnemy[i].rc.left + 25, vEnemy[i].rc.top + 20, vEnemy[i].angleToPlayer);
						if (vEnemy[i].isFire)vEnemy[i].gunsparkImg->rotateFrameRender(getMemDC(), vEnemy[i].gunsparkX, vEnemy[i].gunsparkY, vEnemy[i].frameIdx, vEnemy[i].frameY, vEnemy[i].angleToPlayer);
					}
				...
				case E_TURRET:
					vEnemy[i].idleImg->frameRender(getMemDC(), vEnemy[i].rc.left, vEnemy[i].rc.top, vEnemy[i].frameIdx, vEnemy[i].frameY);
					if (vEnemy[i].isFire)vEnemy[i].gunsparkImg->rotateFrameRender(getMemDC(), vEnemy[i].rc.left - 30, vEnemy[i].rc.top + 13, vEnemy[i].frameIdx, 1, vEnemy[i].angleToPlayer);
					break;
				}
			}
		}
	...
	if (isAllDead && DATA->getCurrentClear()!= STAGE3_CLEARED && DATA->getCurrentClear() != STAGE4_CLEARED && DATA->getCurrentClear() != STAGE5_RESET)
	{
		go.img->render(getMemDC(), go.x, go.y);
		IMAGEMANAGER->findImage("hud_go_font")->render(getMemDC(), 1150, 130);
	}
...
void enemyClass::enemyDoorDead(int i)
{
	...
	if (vEnemy[i].realDead == false)
	{
		vEnemy[i].x += (vEnemy[i].speed * 9);
	}
...
{
	sEnemy.isExplosionDead = false;
	sEnemy.gravity = 0;
	sEnemy.deadAngle = 1;
	sEnemy.realDead = false;
	sEnemy.alreadyDead = false;
	sEnemy.foundedCnt = 0;
	sEnemy.x = _x;
	sEnemy.y = _y;
	sEnemy.isRight = _set;
	...
	case E_BOLD:
		...
		case E_HURT:
			...
			if (vEnemy[i].isRight)
			{
				...
				{
					vEnemy[i].frameIdx++;
					if (vEnemy[i].frameIdx == 2) vEnemy[i].realDead = true;
					if (vEnemy[i].frameIdx == 11)SOUNDMANAGER->play("ground", 0.6f);
					if (vEnemy[i].frameIdx > 14)
					{
						vEnemy[i].realDead = true;
			...
			else
			{
				...
				{
					vEnemy[i].frameIdx--;
					if (vEnemy[i].frameIdx == 12) vEnemy[i].realDead = true;
					if (vEnemy[i].frameIdx == 2)SOUNDMANAGER->play("ground", 0.6f);
					if (vEnemy[i].frameIdx < 1)
					{
						vEnemy[i].realDead = true;
						vEnemy[i].frameIdx = 0;
	...
	case E_POMP:
		...
		case E_HURT:
			...
			if (vEnemy[i].isRight)
			{
				if (vEnemy[i].frameCnt % vEnemy[i].frameTum == 0)
				{
					vEnemy[i].frameIdx++;
					if (vEnemy[i].frameIdx == 2) vEnemy[i].realDead = true;
					if (vEnemy[i].frameIdx == 12)SOUNDMANAGER->play("ground", 0.6f);
					if (vEnemy[i].frameIdx > 14)
					{
						vEnemy[i].realDead = true;
						vEnemy[i].frameIdx = 15;
		...
		case E_ATTACK:
			...
			if (vEnemy[i].isRight)
			{
				...
				{
					vEnemy[i].frameIdx++;
					if (vEnemy[i].frameIdx == 3)SOUNDMANAGER->play("e_swing", 0.5f);
			...
			else
			{
				...
				{
					vEnemy[i].frameIdx--;
					if (vEnemy[i].frameIdx == 2)SOUNDMANAGER->play("e_swing", 0.5f);
	...
	case E_GRUNT:
		...
		case E_HURT:
			...
			if (vEnemy[i].isRight)
			{
				if (vEnemy[i].frameCnt % vEnemy[i].frameTum == 0)
				{
					vEnemy[i].frameIdx++;
					if (vEnemy[i].frameIdx == 2) vEnemy[i].realDead = true;
					if (vEnemy[i].frameIdx == 13)SOUNDMANAGER->play("ground", 0.6f);
					if (vEnemy[i].frameIdx > 14)
					{
						vEnemy[i].realDead = true;
						vEnemy[i].frameIdx = 15;
			...
			else
			{
				if (vEnemy[i].frameCnt % vEnemy[i].frameTum == 0)
				{
					vEnemy[i].frameIdx--;
					if (vEnemy[i].frameIdx == 12) vEnemy[i].realDead = true;
					if (vEnemy[i].frameIdx == 1)SOUNDMANAGER->play("ground",0.6f);
					if (vEnemy[i].frameIdx < 0)
					{
						vEnemy[i].realDead = true;
						vEnemy[i].frameIdx = 0;
		...
		case E_ATTACK:
			...
			if (vEnemy[i].isRight)
			{
				...
				{
					vEnemy[i].frameIdx++;
					if (vEnemy[i].frameIdx == 4)SOUNDMANAGER->play("e_punch", 0.5f);
			...
			else
			{
				...
				{
					vEnemy[i].frameIdx--;
					if (vEnemy[i].frameIdx == 2)SOUNDMANAGER->play("e_punch", 0.5f);
					if (vEnemy[i].frameIdx < 2)vEnemy[i].isAttack = true;
					else if (vEnemy[i].frameIdx >= 2)vEnemy[i].isAttack = false;
...
void enemyClass::enemyMoveToPlayer(int i)
{
	if (vEnemy[i].distanceToPlayer > vEnemy[i].findRange + DATA->getBgCol().x && vEnemy[i].isAttackReady == false)
	{
		...
		if (abs(vEnemy[i].distanceToPlayer_Y) > 100 && vEnemy[i].foundedCnt > 130)
		{
			vEnemy[i].founded = false;
			vEnemy[i].foundedCnt = 0;
		}
	}
	else
	{
	...
	if (vEnemy[i].isAttackReady == true)
	{
		...
		else if(vEnemy[i].state != E_ATTACK)
		{
			vEnemy[i].foundedCnt = 0;
			vEnemy[i].state = E_IDLE;
