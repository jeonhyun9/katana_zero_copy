	...
	if (boss.isGenrateCoolTime == false)
	{
		...
		case B_WALLJUMP:
			if (boss.isRight)
			{
				...
				boss.wallJumpImg->frameRender(getMemDC(), boss.rc.left - 34, boss.rc.top - 16, boss.frameIdx, boss.frameY);
			}
			else
			{
				if (boss.frameIdx != 0)
				...
				boss.wallJumpImg->frameRender(getMemDC(), boss.rc.left - 59, boss.rc.top - 16, boss.frameIdx, boss.frameY);
			}
			break;
		case B_LAND:
...
void bossClass::setBossState(enum_BossState _state)
{
	...
	case B_SHOOT:
		...
		break;
	case B_IN:
		SOUNDMANAGER->play("teleport",0.5f);
		boss.isRight ? boss.frameIdx = 0 : boss.frameIdx = 4;
		break;
	case B_OUT:
		boss.stateCnt = 0;
